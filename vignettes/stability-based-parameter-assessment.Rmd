...
```{r}
nn_importance_object2 <- assess_nn_stability(
  embedding = cuomo@reductions$pca@cell.embeddings,
  ...
  ecs_thresh = 1,
  ncores = n_cores,
  seed_sequence = seq(from = 1, by = 100, length.out = 30),
  algorithm = 1
)
```
...
plot_clustering_overall_stability(clustering_stability, "resolution", min)#function(x) { quantile(x, probs = 0.25)[[1]]})
```
...
plot_k_resolution_corresp(clustering_stability, dodge_width = 0.5) +
...
```{r merge_partitions}
resolution_gridsearch_thresh_99 <- merge_partitions(clustering_stability,
  ecs_thresh = 0.99,
  ncores = n_cores
)
```
