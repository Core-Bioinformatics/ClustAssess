...
```{r eval = FALSE}
install.packages(c("devtools", "readr", "RhpcBLASctl"))
devtools::install_github("Core-Bioinformatics/ClustAssess@release-0.4.0")
```
...
list_mbs <- lapply(1:10, function(i) { sample.int(4, 5, replace = TRUE)})
print(list_mbs)
...
```{r load}
meta.path <- "https://bioinf.stemcells.cam.ac.uk/shiny/clustassess_1gmg5bq/cuomo_metadata.csv"
counts.path <- "https://bioinf.stemcells.cam.ac.uk/shiny/clustassess_1gmg5bq/all_genes_with_RP_MT.csv.gz"
...
```{r save}
saveRDS(cuomo, "cuomo_object.rds")
```
...
wrap_plots(lapply(
  c(
    DimPlot(cuomo, group.by = c("day.ident", "donor.ident"), combine = F),
...
```{r}
cuomo <- FindNeighbors(cuomo)
clustering_list <- lapply(seq(from = 1, by = 100, length.out = 10), function(i) {
  cuomo <- FindClusters(cuomo, algorithm = 3, random.seed = i)
  as.integer(cuomo$seurat_clusters)
})
```

```{r}
ecc_value <- element_consistency(clustering_list)
print(fivenum(ecc_value))
```

```{r}
ggplot(data.frame(raw_umap), aes(x = UMAP_1, y = UMAP_2, colour = ecc_value)) + geom_point() + theme_classic()
ggplot(data.frame(raw_umap), aes(x = UMAP_1, y = UMAP_2, colour = clustering_list[[1]])) + geom_point() + theme_classic()
ggplot(data.frame(raw_umap), aes(x = UMAP_1, y = UMAP_2, colour = clustering_list[[2]])) + geom_point() + theme_classic()
...
pca_feature_stability_object <- mapply(c,
  assess_feature_stability(
    data_matrix = cuomo@assays[["RNA"]]@scale.data,
...
plot_feature_stability_ecs_facet(pca_feature_stability_object, resolution = 0.8)
```
...
nn_conn_comps_object <- get_nn_conn_comps(
    embedding = cuomo@reductions$pca@cell.embeddings,
...
The stability of the methods is evaluated using the Element-Centric Consistency (ECC), which is applied on the partition list obtained over multiple runs. The following summary plot underlines the variation of the consistency as the resolution parameter increases. Above each boxplot, we display the number of clusters corresponding to the most frequent partition for the clustering method with the specific resolution parameter. Increasing the resolution leads to more clusters; in addition, we note consistently high stability for all four algorithms, although Leiden seems to perform noticeably worse for the resolution value of 0.7.
...
The stats can be further summarised using functions such as the mean or the median and the overall displayed can be visualised in the following plot. This utility of this figure is proven on the task of choosing the clustering method that is overall the most stable. In our case, both SLM and Louvain refined are the most stable.
...
```{r k_res_corr_1, fig.width=15, fig.height=5}
plot_k_resolution_corresp(clustering_stability, dodge_width = 0.5, summary_function = min) +
  plot_annotation(title = "resolution - k correspondence with ecs threshold = 1")
plot_k_resolution_corresp(clustering_stability, colour_information = "freq_k") +
  plot_annotation(title = "resolution - k correspondence with ecs threshold = 1")
```
...
plot_k_n_partitions(clustering_stability) + plot_annotation(title = "k - # partitions correspondence with ecs threshold = 1")
plot_k_n_partitions(clustering_stability, colour_information = "freq_part") + plot_annotation(title = "k - # partitions correspondence with ecs threshold = 1")
```
