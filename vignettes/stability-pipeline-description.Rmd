...
is_rhpcblas <- require("RhpcBLASctl", quietly = TRUE)
if (!is_rhpcblas) {
...
is_patchwork <- require("patchwork", quietly = TRUE)
if (!is_patchwork) {
...
wrap_plots(lapply(
    c(
        ...
        FeaturePlot(pbmc3k, features = c("nFeature_RNA", "nCount_RNA", "percent.mito", "percent.rp"), combine = FALSE)
    ),
    function(x) {
...
```{r resolution_importance, warning=FALSE}
