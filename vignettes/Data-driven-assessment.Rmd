---
title: "Data-driven summary assessment vignette"
---

```{r, warning = F, echo = F, message = F,cache=F}
library(Seurat)
...
# source("../ClustAssess/R/ECS.R") delete after reaching a stable form of the package
# source("../ClustAssess/R/data_driven_assessment.R")
```

In this notebook we illustrate a data driven pipeline for the assessment of optimal clustering parameters for a given dataset. The case study is based on a subset of cells from the Cuomo et al 2020 (Single-cell RNA-sequencing of differentiating iPS cells reveals dynamic genetic effects on gene expression [ref](https://www.nature.com/articles/s41467-020-14457-z)) dataset. Specifically, 6 donors and 4 time points were chosen, comprising 1880 cells and 35430 expressed genes. The input is provided as a 35430 x 1880 expression level matrix containing the raw quantifications (in csv format).
...
The data is normalized using the Seurat's `SCTransform` method. 

```{r, warning=F}
cuomo = SCTransform(cuomo, do.scale = T, return.only.var.genes=FALSE, verbose=FALSE)



features = dimnames(cuomo@assays$SCT)[[1]]
var_features = cuomo@assays[["SCT"]]@var.features

cuomo = RunPCA(cuomo, npcs = 30, approx = F, verbose = F, features = intersect(rownames(cuomo@assays$SCT)[order(Matrix::rowSums(cuomo@assays$SCT), decreasing=TRUE)][1:3000], cuomo@assays$SCT@var.features))
cuomo = RunUMAP(cuomo,reduction = "pca", dims = 1:30, n.neighbors = 30, min.dist = 0.3, metric = "cosine", verbose = F)
raw_umap = cuomo@reductions$umap@cell.embeddings #you ues it only here?
```

# QC

```{r qc, fig.width=15, fig.height = 8, include = T, echo = F}
plotlist = list(
  ggplot(data.frame(raw_umap), aes(x = UMAP_1, y = UMAP_2)) +
    geom_point(aes(color = cuomo@meta.data$day.ident)) +
    theme_classic() +
    labs(color = "timepoint"),
  ggplot(data.frame(raw_umap), aes(x = UMAP_1, y = UMAP_2)) +
    geom_point(aes(color = cuomo@meta.data$donor.ident)) +
    theme_classic() +
    labs(color = "donor"),
  ggplot(data.frame(raw_umap, nCount = cuomo@meta.data$nCount_RNA), aes(x = UMAP_1, y = UMAP_2, color = nCount)) +
    geom_point() +
    theme_classic() + 
    scale_color_viridis_c(),
  ggplot(data.frame(raw_umap, nFeature = cuomo@meta.data$nFeature_RNA), aes(x = UMAP_1, y = UMAP_2, color = nFeature)) +
    geom_point() +
    theme_classic() + 
    scale_color_viridis_c(),
  ggplot(data.frame(raw_umap, percent_mito = cuomo@meta.data$percent.mito), aes(x = UMAP_1, y = UMAP_2, color = percent_mito)) +
    geom_point() +
    theme_classic() + 
    scale_color_viridis_c(),
  ggplot(data.frame(raw_umap, percent_rp = cuomo@meta.data$percent.rp), aes(x = UMAP_1, y = UMAP_2, color = percent_rp)) +
    geom_point() +
    theme_classic() + 
    scale_color_viridis_c())

annotate_figure(ggarrange(plotlist = plotlist, ncol = 3, nrow = 2, labels = c("A", "B", "C", "D", "E", "F")), 
                bottom = text_grob("Figure 1. QC overview for the Cuomo dataset. \n(A) time point induced separation of cells; \n (B) distributions of cells per donor, we notice a good overlap across all 6 donors, supporting the assumption of no significant batch effect. \n (C) –(F) distributions of sequencing depths \n (C), number of features (D), and percentages of reads incident to mitochondrial genes (E) and ribosomal genes (F) illustrated using the colour gradient. \nNo significant effects are noted.", color = "black", face = "italic", size = 13))
...
The following analysis is performed by varying the seed multiple times. As runs on different seeds are independent, the code can be parallelized. The `n_cores` user-defined parameter indicates the number of parallel instances of the pipeline. Setting the value of 1 will result in a sequential run of all iterations. We encourage the usage of a higher number of cores when possible, as it will decrease the runtime of the assessment pipeline. We note that the number of cores should be chosen carefully, as each newly created process will add to the memory usage.
...
* **intersection between highly variable and most abundant genes**: this set is obtained by intersecting a subset of the most abundant genes and the whole highly variable feature set. The intersection tends to have a smaller number of genes compared to the initial sets.

```{r}
most_abundant_genes = rownames(cuomo@assays$SCT)[order(Matrix::rowSums(cuomo@assays$SCT), decreasing=TRUE)]
...
`get_feature_stability_object` is a function that explores the stability of a feature set. The input consists of a normalized expression matrix, that is summarised using either PCA or UMAP. The obtained embedding is then processed using the Leiden community detection method (on default values as described in the Seurat pipeline). This process is iterated on different initial seeds. The method outputs a list containing an UMAP embedding, the cell assignment corresponding to the most frequent partition and the Element-Centric Consistency across all communities obtained throughout the iterations.
...
```{r}
pca_feature_stability_object = c(get_feature_stability_object(data_matrix = t(cuomo@assays[["SCT"]]@scale.data),
                                                              ...
                                                              ecs_thresh = 1,
                                                              algorithm = 1),
                                 get_feature_stability_object(data_matrix = t(cuomo@assays[["SCT"]]@scale.data),
...
For the Cuomo case study, based on the UMAP plots, we settle on using the intersection between most abundant and highly variable genes, as it revealed a sensible trade-off that captures the benefits of both sets.

```{r}
cuomo = cuomo[most_abundant_genes[1:3000], ] # Seurat will perform the intersection automatically
...
* **base embedding**: the graph can be built on either the PCA or the UMAP embedding (using the expression matrix isn’t recommended, as calculating the distances would significantly increase the runtime)
* **the number of neighbours**
* **the graph type**: the graph can be either unweighted (NN case) or based on Shared-Nearest Neighbours (SNN, weighted option). For the later, the weights are computed using the Jaccard Similarity Index (JSI) between the neighbourhoods of two cells.




`get_nn_conn_comps` is a method used to link the number of neighbours and the number of connected components (a connected component is a subgraph within which there exists a path between every pair of nodes). The method accepts as input both PCA and UMAP reductions; the distribution of the number of connected components obtainable for the given number of neighbours is built on different random seeds. The output is a list containing, for each number of neighbours, an array with the number of connected components obtained on different seeds. As before, the objects can be concatenated to facilitate comparisons between different configurations and reductions.
...
nn_conn_comps_object = c(get_nn_conn_comps(object = cuomo@reductions$pca@cell.embeddings,
                                           ...
                                           n_neighbors = 30,
                                           metric = "cosine"),
                         get_nn_conn_comps(object = cuomo@assays[["SCT"]]@scale.data,
                                           n_neigh_sequence = c(c(1,2,3,4), seq(from = 5, to = 30, by = 5)),
                                           n_repetitions = n_repetitions,
                                           graph_reduction_type = "PCA",
                                           ncores = n_cores,
...
The following plot describes the covariation between the number neighbours and the number of connected components obtained using both PCA and UMAP reductions as base for graph building. As the number of neighbours increases, the number of connected components decreases (this is an expected result, as increasing the number of neighbours result in a better connected graph). Another comparison may be performed on the PCA and UMAP embeddings to assess graph connectivity. We note that, when using UMAPs, a better separation of the cells is achieved; this is not observed for PCAs, for which a quick convergence to a connected graph (which is a graph where there exists a path between every two nodes) is obtained. The link between the number of connected components and the functional interpretation of the results is that the former could be interpreted as the minimum number of clusters that could be obtained with the given number of neighbours e.g. for the UMAP case, to obtain 5 communities from the clustering step, we would require at least 15 nearest neighbours.
...
nn_importance_object = mapply(c,
                              get_nn_importance(object = cuomo@assays[["SCT"]]@scale.data,
...
The final step in a standard single-cell analysis pipeline is applying a graph-based clustering method. Choosing a community detection algorithm has a significant impact on the partitioning results. Using the `get_clustering_difference_object` we assess the stability and reproducibility of results obtained using various graph clustering methods available in the Seurat package, i.e. Louvain, Louvain refined, SLM and Leiden.

```{r}
cuomo = RunPCA(cuomo, npcs = 30, approx = F, verbose = F)
cuomo = RunUMAP(cuomo,reduction = "pca", dims = 1:30, n.neighbors = 30, min.dist = 0.3, metric = "cosine", verbose = F)
adj_matrix = FindNeighbors(cuomo@reductions$pca@cell.embeddings, k.param = 25, nn.method = "rann", verbose = F)$snn

start_time_clustering_method = Sys.time()
clustering_diff_obj = get_clustering_difference_object(graph_adjacency_matrix = adj_matrix,
...
The stability of the methods is evaluated using the Element-Centric Consistency (ECC), which is applied on the partition list obtained over the multiple runs. The following summary plot underlines the variation of the consistency as the resolution parameter increases. Above each boxplot we display the number of clusters corresponding to the most frequent partition for the clustering method with the specific resolution parameter. Increasing the resolution leads to more clusters; in addition, we note a similarity in outputs for all three algorithms (an exception is noted when resolution is 0.9). For the exception case, SLM produced more stable results.

```{r, fig.width = 15, fig.height=6}
plot_clustering_difference_boxplot(clustering_diff_obj)
```

To further underline the areas of instability, we provide an additional plot where we display, on the same UMAPs as before, the Element-Centric Consistency (or ECC) score of the partitions obtained for each resolution parameter with each clustering method. We notice that for a higher resolution, all three algorithms are unstable in the same area, namely the large group of cells from bottom left.
...
The main parameter that affects the clustering output is the resolution parameter. As previously suggested, its values can affect the number of clusters. The `wrapper_gridsearch` focuses on assessing the effect of a range of values for the resolution parameter on the stability of the output. Besides the resolution, the user can input multiple values of the following parameters: number of neighbours, the graph type and the clustering method. The function will return a list associated to each configuration (a configuration is a data-structure showcasing a set of fixed values for the parameters that are optimised). For each resolution value, the list will contain multiple sublists of partitions corresponding to specific number of clusters.

```{r}
resolution_gridsearch = wrapper_gridsearch(embedding = cuomo@reductions$pca@cell.embeddings,
                                           resolution = seq(from = 0.5, to = 1, by = 0.1),
                                           n_neigh = 30,
                                           n_repetitions = n_repetitions,
